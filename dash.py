...
def extract_content(url: str) -> str:
    """Extract text content from a URL using cloudscraper with fallbacks to Wayback Machine."""
       ...
       # try:
        ...
        try:
                payload = {
                    ...
                    'country_code': 'us',  # Target US servers
                    'autoparse': 'true'  # Enable auto-parsing
                }
                ...
                try:
                    ...
                    if response.status_code == 200:
                        wayback_date = re.search(r'web/(\d+)/', response.url)
                        if wayback_date:
                            date_str = wayback_date.group(1)
                            formatted_date = f"{date_str[6:8]}-{date_str[4:6]}-{date_str[0:4]}"
                            source_info = f"Wayback Machine snapshot [{formatted_date}]"
                        else:
                            source_info = "Wayback Machine snapshot [date unknown]"
                        st.info(f"Using {source_info} for {url}")
                        html = response.content
                    else:
                        raise Exception("No Wayback snapshot available")
                except Exception as e:
                    st.warning(f"Wayback Machine failed for {url}. Trying Selenium...")
                    
                    # 5. Final fallback to Selenium
                    try:
                        html = extract_with_selenium(url)
                        source_info = "Selenium-extracted version"
                    except Exception as e:
                        st.error(f"All methods failed to extract from {url}: {str(e)}")
                        return ""

    # Store source info in session state cache
    if url not in st.session_state.cache:
        st.session_state.cache[url] = {}
    st.session_state.cache[url]['source'] = source_info
...
def extract_with_selenium(url: str) -> str:
    ...
    try:
        ...
        driver.get(url)
        time.sleep(5)  # Give JS time to load, adjust based on need
        html = driver.page_source
        driver.quit()
        return html
    except WebDriverException as e:
        raise RuntimeError(f"Selenium error: {str(e)}")
